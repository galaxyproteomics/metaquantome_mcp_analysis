...
Here, the peptides are annotated with their lowest common ancestor as NCBI taxonomy names. Unlike names, NCBI taxIDs are unambiguous and not affected by differences in capitalization or spelling errors. So, taxIDs are *highly* recommended. 
...
Note that missing data is represented as `NA`. In general, metaQuantome treats `0`, `NA`, and `NaN` as missing data.
...
metaQuantome has a separate module for downloading the necessary databases. Let's first get the help text for the module and make sure everything is installed correctly:
...
As detailed in the manuscript, there are several questions that metaQuantome can be used to explore. Let's go through a few of these.
